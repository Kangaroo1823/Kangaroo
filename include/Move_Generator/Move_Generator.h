...
namespace Kangaroo::Move_Generator
{
    ...
    class Move_Generator : public Pawn_Move_Generator<status, CallbackType>
        ...
        , public Queen_Move_Generator<status, CallbackType>
    ...
    public:
        constexpr explicit Move_Generator(Chess_Board* board)
            ...
            , Queen_Move_Generator<status, CallbackType>(board)
        ...
        , Queen_Move_Generator<status, CallbackType>(pac_gen)
        {
        }
        ...
        template<typename ... Args>
        [[nodiscard]] _ForceInline constexpr std::size_t generate_queen_movements(Args... args) {
            using enum Move_Generation_Mode;
            std::size_t moves = 0;
            moves += this->template generate_queen_moves<Normal_Move_Generation>(args...);
            moves += this->template generate_queen_moves<Pin_HV_Move_Generation>(args...);
            moves += this->template generate_queen_moves<Pin_D_Move_Generation>(args...);
            return moves;
        }
        ...
        [[nodiscard]] _ForceInline constexpr std::size_t generate_moves(Args... args)
        {
            ...
            moves += this->generate_queen_movements(args...);
