...
TEST(MagicNumbers, magic_numbers) {
    ...
    for (const auto &position: All_Positions) {
        ...
        bool flag = check_magic_number<Chess_Pieces::Rook>(position, Constants::rook_magic_numbers[std::to_underlying(position)]);
        ASSERT_TRUE(flag);

        // cppcheck-suppress unreadVariable
        bool flag2 = check_magic_number<Chess_Pieces::Bishop>(position, Constants::bishop_magic_numbers[std::to_underlying(position)]);
        ASSERT_TRUE(flag2);
    }
}
