...
namespace Kangaroo::Knight_Move_Generation::PinCheck1 {
    Callback_Template_Inline(Pawn_Movement_Generator_Test2_Callback, const std::array<Chess_Board,
        ...
        if (!pac_gen->get_board()->is_state_consistent()) {
            ...
            ss << "   black pawns:    " << format_bitboard(bitboard_for(*pac_gen->get_board(), Black, Pawn))
                    <<
                    std::endl;
            ss << "   white knights:  " << format_bitboard(
                        bitboard_for(*pac_gen->get_board(), White, Knight)) <<
                    std::endl;
            ss << "   black knights:  " << format_bitboard(
                        bitboard_for(*pac_gen->get_board(), Black, Knight)) <<
                    std::endl;
            ss << "   white bishops:  " << format_bitboard(
                        bitboard_for(*pac_gen->get_board(), White, Bishop)) <<
                    std::endl;
            ss << "   black bishops:  " << format_bitboard(
                        bitboard_for(*pac_gen->get_board(), Black, Bishop)) <<
                    std::endl;
            ...
            ss << "   black rooks:    " << format_bitboard(bitboard_for(*pac_gen->get_board(), Black, Rook))
                    <<
                    std::endl;
            ss << "   white queens:   " << format_bitboard(
                        bitboard_for(*pac_gen->get_board(), White, Queen)) <<
                    std::endl;
            ss << "   black queens:   " << format_bitboard(
                        bitboard_for(*pac_gen->get_board(), Black, Queen)) <<
                    std::endl;
            ...
            ss << "   black king:     " << format_bitboard(bitboard_for(*pac_gen->get_board(), Black, King))
                    <<
                    std::endl;
            ss << "   white pieces:   " << format_bitboard(all_pieces_for(*pac_gen->get_board(), White)) <<
                    std::endl;
            ss << "   black pieces:   " << format_bitboard(all_pieces_for(*pac_gen->get_board(), Black)) <<
                    std::endl;
            ss << "   all pieces:     " << format_bitboard(total_pieces_for(*pac_gen->get_board())) <<
                    std::endl;
            ss << "   en passant sq.: " << format_bitboard(en_passant_square_for(*pac_gen->get_board())) <<
                    std::endl;
            ss << "-------------------------------------------------------------------------" << std::endl;
            throw std::runtime_error(ss.str());
        }
    ...
    [[maybe_unused]] std::array<Chess_Board, 2> new_boards = {
        ...
        */
        Kangaroo::Chess_Board(std::array<Bitboard, 15>{
            /* white pawns    */ 0x0000000000000000, /* white knights */ 0x0000000200000000,
            /* white bishops */ 0x0000000000000000,
            /* white rooks    */ 0x0000000000000000, /* white queens  */ 0x0000000000000000,
            /* white king    */ 0x0000000100000000,
            /* black pawns    */ 0x0000000000000000, /* black knights */ 0x0000000000000000,
            /* black bishops */ 0x0000000000000000,
            /* black rooks    */ 0x0000008000000000, /* black queens  */ 0x0000000000000000,
            /* black king    */ 0x0000000000000000,
            /* en passant sq. */ 0x0000000000000000, /* half move num */ 0x0000000000000000,
            /* full move num */ 0x0000000000000001
        }),
        /*
...
*/
        Kangaroo::Chess_Board(std::array<Bitboard, 15>{
            /* white pawns    */ 0x0000000000000000, /* white knights */ 0x0000002000000000, /* white bishops */
            0x0000000000000000,
            ...
            0x0000000000000000,
            /* black rooks    */ 0x0000008000000000, /* black queens  */ 0x0000000000000000, /* black king    */
            0x0000000000000000,
...
1    .  .  .  .  .  .  .  .
   ...
   bitboard as 64 bit integer: */
     ...
     */
    TEST(Knight_Move_Generation, Check1) {
        ...
        Chess_Board board{};
        const auto status = board.reset_board("8/8/8/K6r/3N4/8/8/8 w kqKQ - 0 1");
